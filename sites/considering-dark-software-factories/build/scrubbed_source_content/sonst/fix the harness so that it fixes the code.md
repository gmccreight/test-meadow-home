^ fix the [[agent harness]] so that it fixes the <span class="link-not-tracked">link not tracked</span>
...
When a [[agentic coding|coding agent]] forgets a unit test, drifts architecturally, or misses a verification step, the durable fix is usually a [[mechanisms for making agents more reliable|mechanism]]: a better prompt, [[agent skill]], [[custom linting rule]], <span class="link-not-tracked">link not tracked</span>, reviewer agent, CI check, or observability hook. The point is to convert the failure into a repeatable constraint or <span class="link-not-tracked">link not tracked</span> so future generated code improves automatically.
