...
:
As just one example, in prior work we found that [[AI unit - Claude Sonnet 4.5]] would wrap up tasks prematurely as it sensed its context limit approaching—a behavior sometimes called [[context anxiety]] We addressed this by adding context resets to the harness. But when we used the same harness on Claude Opus 4.5, we found that the behavior was gone. The resets had become dead weight.
......
